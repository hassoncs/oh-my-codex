    ...
    it('materializes ralplan consensus handoff artifacts when ralplan is skipped', async () => {
      ...
      await writeFile(join(stateDir, 'ralplan-state.json'), JSON.stringify({
        ...
        ralplan_consensus_gate: {
          complete: true,
          ralplan_architect_review: { agent_role: 'architect', verdict: 'approve', summary: 'architect ok', sequence_index: 1 },
          ralplan_critic_review: { agent_role: 'critic', verdict: 'approve', summary: 'critic ok', sequence_index: 2 },
        },
      }));
